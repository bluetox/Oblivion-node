...
/// # Ok(())}
/// ```
pub fn keypair<R>(rng: &mut R, seed: Option<(&[u8], &[u8])>) -> Keypair
where
    R: RngCore + CryptoRng,
{
    ...
    crypto_kem_keypair(&mut public, &mut secret, rng, seed);
...
/// # Ok(())}
/// ```
pub fn encapsulate<R>(pk: &[u8], rng: &mut R, seed: Option<&[u8]>) -> Encapsulated 
  where R: CryptoRng + RngCore
{
  ...
  crypto_kem_enc(&mut ct, &mut ss, pk, rng, seed);
  Ok((ct, ss))
}
